...
Let $S:X\to\strongdual X$ be the Fredholm operator induced by a continuous alternating form on a reflexive
real Banach space and put $F=\ker S$.  There is a closed complement $X=F\oplus Y$.  Restriction
and the bounded projections give
\[
...
$t\mapsto S_t:X\to\strongdual X$ be norm-continuous
on $J$ in its subspace topology, with every $S_t$ induced by a continuous alternating form and
$S_{t_0}$ Fredholm.  Use
Lemma~\ref{lem:kernel-splitting} to write
\[
...
injective linear map $m_t:F\to X$ and a continuous alternating form $c_t$ on $F$ such that the
restriction of $m_t$ is a linear isomorphism
\[
